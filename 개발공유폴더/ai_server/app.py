...
def upload_file():
    ...
    try:
        ...
        # --- 비동기로 실행할 함수들 정의 ---
        thread_lgvm = threading.Thread(target=ai1.lgvm, args=(file_content[0], engine))
        thread_rf = threading.Thread(target=ai2.rd_forest, args=(file_content[1], engine))
        thread_svm = threading.Thread(target=ai3.svm, args=(file_content[2], engine))
        thread_xgb = threading.Thread(target=ai4.xgboost, args=(file_content[3], engine))
